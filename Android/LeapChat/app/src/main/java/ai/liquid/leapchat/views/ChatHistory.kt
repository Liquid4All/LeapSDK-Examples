...
@Composable
fun ChatHistory(history: List<ChatMessageDisplayItem>, modifier: Modifier = Modifier) {
  val scrollState = rememberLazyListState()
  LaunchedEffect(history) { scrollState.animateScrollToItem(history.count()) }
  ...
  ChatHistory(
    listOf(
      ...
      ChatMessageDisplayItem(
        ChatMessage.Role.ASSISTANT,
        text = "Hello user!",
        reasoning = "I should be friendly",
      ),
      ChatMessageDisplayItem(
        ChatMessage.Role.USER,
        text = "Are you really a robot or a human?",
        reasoning = null,
      ),
      ChatMessageDisplayItem(
        ChatMessage.Role.ASSISTANT,
        text = "I am a language model.",
        reasoning = "I should be accurate",
      ),
    )
  )
}
