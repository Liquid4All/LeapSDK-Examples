...
class MainActivity : ComponentActivity() {
  ...
  private val modelRunner: MutableLiveData<ModelRunner> by lazy { MutableLiveData<ModelRunner>() }
  ...
  private val isInGeneration: MutableLiveData<Boolean> by lazy { MutableLiveData<Boolean>(false) }

  private val isToolEnabled: MutableLiveData<Boolean> by lazy { MutableLiveData<Boolean>(false) }
  ...
  override fun onDestroy() {
    super.onDestroy()
    // Unload on a fresh scope: lifecycleScope is cancelled by the time onDestroy fires,
    // so launches there would never run.
    val runner = modelRunner.value
  ...
  fun MainContent() {
    ...
    val chatMessageHistory: List<ChatMessageDisplayItem> by
      chatMessageHistory.observeAsState(listOf())
    ...
    Scaffold(
      modifier =
        Modifier.fillMaxSize()
          .windowInsetsPadding(NavigationBarDefaults.windowInsets.union(WindowInsets.ime)),
      bottomBar = {
        Box {
          if (modelRunnerInstance == null) {
            ...
            Column {
              TextField(
                ...
                modifier = Modifier.padding(4.dp).fillMaxWidth(1.0f).testTag("InputBox"),
                enabled = !isInGeneration.value,
              )
              Row(horizontalArrangement = Arrangement.End, modifier = Modifier.fillMaxWidth(1.0f)) {
                val isToolEnabledState by isToolEnabled.observeAsState(false)
                Button(onClick = { isToolEnabled.value = !isToolEnabledState }) {
                ...
                Button(onClick = { job?.cancel() }, enabled = isInGeneration.value) {
      ...
      Box(
        Modifier.padding(innerPadding).focusRequester(chatHistoryFocusRequester).focusable(true)
      ) {
    ...
    lifecycleScope.launch {
      try {
              ...
              notificationConfig =
                LeapModelDownloaderNotificationConfig.build {
                  notificationTitleDownloading =
                    getString(R.string.notification_downloading_chat_model)
                  notificationTitleDownloaded = getString(R.string.notification_chat_model_ready)
                },
            )
        }
        ...
        if (currentStatus is LeapModelDownloader.ModelDownloadStatus.NotOnLocal) {
          ...
          onStatusChange(getString(R.string.status_starting_download))

          // Observe download progress
          val progressFlow =
            downloaderInstance.observeDownloadProgress(MODEL_NAME, QUANTIZATION_SLUG)
            ...
            .onEach { progress ->
              if (progress != null) {
                ...
                if (downloadStatus is LeapModelDownloader.ModelDownloadStatus.Downloaded) {
                  onStatusChange(getString(R.string.status_download_complete))
                }
              }
            }
            .takeWhile { progress ->
              progress != null ||
                downloaderInstance.queryStatus(MODEL_NAME, QUANTIZATION_SLUG) is
        ...
        conversation
          ...
          .onEach {
            when (it) {
              ...
              is MessageResponse.Error -> throw it.throwable
          ...
          .catch { exception ->
            Log.e(this@MainActivity::class.java.simpleName, "Error in generation: $exception")
            updateLastAssistantMessage(
              content =
                generateTextBuffer.toString() +
                  "\n\n" +
                  getString(R.string.error_generation_failed, exception.message ?: "unknown"),
              reasoning = generatedReasoningBuffer.toString().takeIf { it.isNotBlank() },
            )
          }
          .collect()
  ...
  private fun processFunctionCalls(functionCalls: List<LeapFunctionCall>) {
    for (call in functionCalls) {
      when (call.name) {
        "compute_sum" -> {
          // LeapFunctionCall.arguments is typed as Map<String, Any?>; the runtime
          // value for an array parameter is a List of the parsed elements. The cast
          // to List<String> is unchecked because of JVM type erasure, but the
          // function descriptor we registered constrains the element type. Items
          // failing toDoubleOrNull() are coerced to 0.0 below as a safety net.
  ...
  private fun loadState(state: Bundle) {
    ...
    if (conversationHistoryJSONString != null) {
      getConversationHistory()
        ...
        ?.let { chatMessageHistory.value = it }
    }
  }
  ...
  companion object {
    const val MODEL_NAME = "LFM2.5-350M"
...
fun ModelLoadingIndicator(
  ...
  var modelLoadingStatusText by remember {
    mutableStateOf(context.getString(R.string.loading_model_content))
  }
  ...
  Column(
    modifier = Modifier.padding(16.dp).fillMaxSize().testTag("ModelLoadingIndicator"),
