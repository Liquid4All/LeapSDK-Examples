...
class MainActivityE2EAssetTests {
  ...
  fun testEndToEndChat() {
    ...
    composeTestRule.waitUntil(timeoutMillis = 5000L) {
      composeTestRule
        .onNode(hasTestTag("AssistantMessageViewText").and(hasText("strawberry", substring = true)))
        .isDisplayed()
    }
    ...
    composeTestRule.onNode(sendButtonMatcher).performClick()
    composeTestRule.waitUntil(timeoutMillis = 5000L) {
      composeTestRule.onAllNodesWithTag("AssistantMessageView").fetchSemanticsNodes().size == 2
    }
  }
