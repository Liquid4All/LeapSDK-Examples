  ...
  override suspend fun provideAgent(
    ...
    val strategy =
      strategy(title) {
        ...
        edge(
          nodeRequestLLM forwardTo
            nodeAssistantMessage transformed
            {
              it.first()
            } onAssistantMessage
            {
        ...
        edge(
          nodeSendToolResultMultiple forwardTo
            nodeAssistantMessage transformed
            {
              it.first()
            } onAssistantMessage
        ...
        edge(nodeAssistantMessage forwardTo nodeFinish transformed { it })
      }
    ...
    return AIAgent(
      ...
      handleEvents {
        onToolCallStarting { ctx -> onToolCallEvent("Tool ${ctx.tool.name}, args ${ctx.toolArgs}") }

        onAgentExecutionFailed { ctx -> onErrorEvent("${ctx.throwable.message}") }
