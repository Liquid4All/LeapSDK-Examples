...
fun WeatherToolScreen(state: WeatherToolState, onEvent: (WeatherToolEvent) -> Unit) {
  Column(
    ...
    if (state.weatherInfo?.isNotEmpty() == true && !state.isLoading) {
      Column(
        modifier =
          Modifier.verticalScroll(rememberScrollState()).padding(vertical = 8.dp).weight(1f)
      ) {
        Text(text = state.weatherInfo)
      }
    } else if (state.isLoading) {
      Text(text = stringResource(R.string.loading), modifier = Modifier.padding(vertical = 8.dp))
    }
