...
object WeatherTools {
  ...
  object AddDatetimeTool : Tool<AddDatetimeTool.Args, AddDatetimeTool.Result>() {
    ...
    override suspend fun execute(args: Args): Result {
      ...
      val period = DateTimePeriod(days = args.days, hours = args.hours, minutes = args.minutes)
  ...
  object WeatherForecastTool : Tool<WeatherForecastTool.Args, WeatherForecastTool.Result>() {
    ...
    data class Args(
      ...
      @property:LLMDescription(
        "The granularity of the forecast: 'daily' for day-by-day forecast or 'hourly' for hour-by-hour forecast. Default is 'daily'."
      )
    ...
    private fun formatDailyForecast(forecast: WeatherForecast, date: String): String {
      ...
      val startDate = date.ifBlank { Clock.System.now().toLocalDateTime(UTC_ZONE).date.toString() }
