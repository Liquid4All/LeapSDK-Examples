...
class YouTubeApiServiceTest {
    ...
    fun `fetchInnerTubeData sends POST with videoId in body and Accept-Language header`() {
        ...
        assert(request.body.readUtf8().contains("testVideoId")) {
            "Request body must contain videoId"
        }
    }
    ...
    fun `fetchVideoPageHtml returns page html`() {
        server.enqueue(
            MockResponse().setResponseCode(200).setBody("<html><body>YouTube</body></html>")
        )
