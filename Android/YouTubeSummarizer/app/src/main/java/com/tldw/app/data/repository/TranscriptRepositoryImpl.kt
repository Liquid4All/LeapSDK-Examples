 ...
 * 2. POST to the InnerTube player endpoint to get captions metadata
 * 3. Find the best matching caption track for the requested language(s)
 * 4. Fetch and parse the transcript XML from the caption track URL
 */
    ...
    override suspend fun fetchTranscript(
        ...
        withContext(Dispatchers.IO) {
            ...
            Transcript(
                videoId = videoId,
                language =
                    captionTrack.name?.runs?.firstOrNull()?.text ?: captionTrack.languageCode,
