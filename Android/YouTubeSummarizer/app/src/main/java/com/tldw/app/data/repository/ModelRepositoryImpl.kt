...
class ModelRepositoryImpl(private val context: Context) : ModelRepository {
    ...
    override fun downloadModel(): Flow<DownloadProgress> = flow {
        ...
        progressFlow
            .takeWhile { progress ->
                val status = dl.queryStatus(Consts.MODEL_NAME, Consts.QUANTIZATION_SLUG)
                progress != null ||
                        status is LeapModelDownloader.ModelDownloadStatus.DownloadInProgress
            }
            .collect { progress ->
                if (progress != null) {
                    val pct =
                    ...
                    emit(
                        DownloadProgress(
                            progress.downloadedSizeInBytes,
                            progress.totalSizeInBytes,
                            pct,
                        )
                    )
                }
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                modelRunner =
                    dl.loadModel(
                        modelName = Consts.MODEL_NAME,
                        quantizationType = Consts.QUANTIZATION_SLUG,
                    )
                Result.success(Unit)
            } catch (e: Exception) {
                Result.failure(e)
    ...
    override fun generateTldr(transcript: String): Flow<String> = flow {
