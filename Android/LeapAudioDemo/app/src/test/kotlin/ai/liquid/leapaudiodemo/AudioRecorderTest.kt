 ...
 *
 * Note: Full recording functionality requires Android runtime and cannot be tested in unit tests.
 * These tests verify the data model and constants. Instrumented tests should cover actual recording
 * behavior.
 */
...
class AudioRecorderTest {
  ...
  fun `AudioCapture should hold samples and sample rate`() {
    ...
    val capture = AudioRecording.AudioCapture(samples, sampleRate)
  ...
  fun `AudioCapture should support different sample rates`() = runTest {
    ...
    val capture16k = AudioRecording.AudioCapture(samples, 16000)
    val capture24k = AudioRecording.AudioCapture(samples, 24000)
    val capture48k = AudioRecording.AudioCapture(samples, 48000)
