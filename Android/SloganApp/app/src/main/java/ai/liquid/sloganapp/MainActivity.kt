...
private const val MODEL_NAME = "Qwen3-0.6B"
...
class MainActivity : ComponentActivity() {
  ...
  private suspend fun loadModel(textView: TextView, modelStatus: TextView): Boolean {
    try {
            ...
            notificationConfig =
              LeapModelDownloaderNotificationConfig.build {
                notificationTitleDownloading =
                  getString(R.string.notification_downloading_slogan_model)
                notificationTitleDownloaded = getString(R.string.notification_slogan_model_ready)
              },
          )
      }
      ...
      if (currentStatus is LeapModelDownloader.ModelDownloadStatus.NotOnLocal) {
        ...
        textView.text = getString(R.string.status_starting_download)
          ...
          .onEach { progress ->
            if (progress != null) {
              ...
              if (downloadStatus is LeapModelDownloader.ModelDownloadStatus.Downloaded) {
                textView.text = getString(R.string.status_download_complete)
              }
            }
          }
          .takeWhile { progress ->
      ...
      textView.text = getString(R.string.error_loading_ai_model_oops_an_error_happened, e.message)
   ...
   * @param generateButton the button for invoking generation. This button will be changed into
   *   "Stop" button when the generation starts.
  ...
  private fun generateContent(
    ...
    generationJob =
      lifecycleScope.launch {
        ...
        conversation
          .generateResponse(USER_PROMPT_TEMPLATE.format(userInput))
          .onEach { chunk ->
            when (chunk) {
              ...
              is MessageResponse.Error -> throw chunk.throwable
              else -> {}
            }
          ...
          .catch { e ->
            textView.text = getString(R.string.error_generation_failed, e.message)
            generateButton.text = getText(R.string.generate_button_label)
            generationJob = null
          }
          .collect()
  ...
  private fun configMainLayout() {
    ...
    button.setOnClickListener {
      if (generationJob?.isActive == true) {
        ...
        if (currentText.startsWith("Generating...") && currentText.trim() == "Generating...") {
          textView.text = getText(R.string.notice_if_generation_stopped)
        } else {
          textView.append(getString(R.string.generation_stopped_appended))
        }
